...
export const TextButtonChunk = (props) => {
    const content = useContext(Context)
    const clickEvent = props.onClick ? props.onClick : null;
    const MouseOverEvent = props.MouseOver ? props.MouseOver : null;
    const MouseOutEvent = props.MouseOut ? props.MouseOut : null;
    ...
    return (
        <>
                <button {...data} onDoubleClick={dbClick} ref={ref} target={target} rel="noopener noreferrer" className={`text-chunk ${ClassName}`} style={style} onMouseEnter={mouseBgcolor} onMouseLeave={mouseOutBgcolor} onClick={click}>
                    {Children}{props.content}
                </button>
        </>
    )
